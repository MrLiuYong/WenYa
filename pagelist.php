...
<table  align="center" cellpadding="0" cellspacing="1" >
    ...
    mysql_select_db('answer');
    mysql_query('set names utf8');
    $Page_size=2;//设置每个页面显示的记录数
    ...
    $init=1;
    $page_len=7;
    $max_p=$page_count;//最大页面数
    $pages=$page_count;

    //判断当前页码
    if(empty($_GET['page'])||$_GET['page']<0)
    {
        $page=1;//如果当前页码为空，就定义为第一页
    }else
    {
        $page=$_GET['page'];//如果当前页不为空，则输出当前页
    }

    $offset=$Page_size*($page-1);// $offset:页面开始位置  $Page_size:结束位置
    $sql="select * from answer limit $offset,$Page_size";//分页查找 select *from 表名 limit $offset.$page_size
